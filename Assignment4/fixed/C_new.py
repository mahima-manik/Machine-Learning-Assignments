...
'''
estimator.fit(X, Y)
pred_train_y = estimator.predict(train_x)
count = 0
for i, j in zip(single_y, pred_train_y):
    if i == j:
        count += 1

print ("Training accuracy", float(count)/len(single_y) * 100.0)
