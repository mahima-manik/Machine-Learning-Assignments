...
def create_dict(review, rating):
    ...
    for i, j in enumerate(temp):
        if i+1 < len(temp):
            bigram = (j, temp[i+1])
            if bigram in vocab:
                update_dict(bigram, rating)
            else:
                vocab[bigram] = [0 for i in range(8)]
                update_dict(bigram, rating)
            count += 1
...
def find_accuracy(p_thetas, p_fais, features, actual):
    ...
    with open(features, 'r') as fx:
        for review in fx:
            ...
            '''For each bigram in that review'''
            r_split = review.split()
            for m, n in enumerate(r_split):
                if m+1 < len(r_split):
                    if (n, r_split[m+1]) in p_thetas:
                        temp = [math.log(i)+j for i, j in zip(p_thetas[(n, r_split[m+1])], temp)]
...
if __name__ == "__main__":
    ...
    with open("imdb/stemmed_train_text.txt") as fx, open("imdb/imdb_train_labels.txt") as fy:
        for review, rating in zip(fx, fy):
            ...
            count = create_dict(review, rating)
            update_fais(p_fais, word_count ,rating, count)
