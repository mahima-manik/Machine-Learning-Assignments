...
# or (niter < len(data_x)/float(batch_size))
while ( (abs(batch_err-prev_err) > 0.015) ):
    ...
    while (count < batch_size):
        ...
        count += 1
    eta = float(1.0/np.sqrt(niter))
    print (eta)
    update_weights(eta, batch_size)
    for jy in model:
        model[jy].activations = [[0]*(model[jy].num_perc)]
