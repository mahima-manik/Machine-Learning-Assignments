...
Testing Accuracy:  63.19444444444444
'''
import numpy as np
...
'''
def delta_j_theta_op (actual_ouputs, actual_inputs):
    global model
    layer_num = len(model)-1
    op_layer = model[layer_num]
    op_layer.deltas = []
    for i, j in zip(actual_ouputs, actual_inputs):
...
def forward_propagate (initial_input, layer_num):
    ...
    model[layer_num].activations = sigmoid(layer0.weights, initial_input)
...
for inst in early_data:
    ...
    if temp[-1] == 6:
        ...
        data_y.append(1)
    arr = list(np.asarray(temp[0:-1])/255.0)
    arr.append(1.0)
    data_x.append(arr)
...
while (abs(batch_err-prev_err) > 0.01):
    prev_err = batch_err
    batch_err = 0
    count = 0
    while (count < batch_size):
        ...
        delta_j_theta_op ([data_y[i]], data_x[i])
        i = int(math.fmod(i+1, len(data_x)))
        count += 1
    update_weights(0.01, batch_size)
    for jy in model:
        model[jy].activations = [[0]*(model[jy].num_perc)]
